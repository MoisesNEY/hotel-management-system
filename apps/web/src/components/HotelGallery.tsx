...
const HotelGallery: React.FC = () => {
  ...
  const { data: facilities } = useListContent('HOME_FEATURES');
  ...
  const { isVisible: galleryVisible, loading: gvLoading } = useSectionVisibility('HOME_GALLERY');
  const { isVisible: mapVisible, loading: mvLoading } = useSectionVisibility('MAIN_LOCATION');
  const { isVisible: contactVisible, loading: cvLoading } = useSectionVisibility('CONTACT_INFO');

  const normalize = (str: string) => str.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  const getContactVal = (key: string) => contactItems.find(c => normalize(c.title || '').includes(normalize(key)));

  const addressItem = getContactVal('direccion'); // coincide con "Dirección", "Direccion"
  const phoneItem = getContactVal('telefono') || getContactVal('celular');
  ...
  if (gvLoading || mvLoading || cvLoading) return null;
  // Solo ocultar si TODO está desactivado
  if (!galleryVisible && !mapVisible && !contactVisible) return null;

  return (
    <section className="bg-white dark:bg-[#1a1a2e] py-20 relative overflow-hidden" id="galeria">
      <div className="max-w-7xl mx-auto px-5">
        ...
        {galleryVisible && (
          ...
          >
            {galleryItems.length > 0 ? (
              ...
              <div className="text-center p-5 text-gray-600 dark:text-gray-400 font-medium bg-gray-50 dark:bg-white/5 rounded-2xl border border-dashed border-gray-200 dark:border-white/10">
                La galería está siendo actualizada. Pronto más fotos.
              </div>
            )}
          </div>
        )}
        ...
        {/* Location & Contact */}
        <div className={`grid grid-cols-1 ${mapVisible && contactVisible ? 'lg:grid-cols-2' : 'lg:grid-cols-1'} gap-12 mt-12`}>
          {contactVisible && (
