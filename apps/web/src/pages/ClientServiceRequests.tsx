      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {showModal && selectedService && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div className="bg-white dark:bg-[#111111] border border-gray-200 dark:border-white/10 w-full max-w-2xl max-h-[90vh] rounded-3xl overflow-hidden relative z-10 animate-in zoom-in-95 duration-200 shadow-2xl flex flex-col">
              ...
              <div className="overflow-y-auto flex-1 custom-scrollbar">
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                  <div className="bg-gray-50 dark:bg-white/5 p-6 rounded-2xl flex items-center gap-6 border border-gray-100 dark:border-white/5">
                    <div className="w-16 h-16 bg-[#d4af37]/10 rounded-2xl flex items-center justify-center text-[#d4af37] shrink-0">
                      {selectedService.imageUrl ? (
                        <img src={selectedService.imageUrl} className="w-full h-full object-cover rounded-2xl" />
                      ) : (
                        <Heart size={32} />
                      )}
                    </div>
                    <div className="flex-1">
                      <p className="text-[10px] text-gray-500 font-bold tracking-widest uppercase mb-1">Servicio seleccionado</p>
                      <p className="text-xl font-bold text-gray-900 dark:text-white">{selectedService.name}</p>
                      <p className="text-[#d4af37] font-bold text-lg">${selectedService.cost}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-semibold text-gray-500 dark:text-gray-400 flex items-center gap-2">
                        <Clock size={14} className="text-[#d4af37]" /> Horario del Servicio
                      </label>
                      <div className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl py-4 px-4 text-gray-600 dark:text-gray-300 font-medium text-sm">
                        {selectedService.startHour || '08:00'} - {selectedService.endHour || '22:00'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50 dark:bg-[#d4af37]/5 border border-blue-100 dark:border-[#d4af37]/10 p-6 rounded-2xl flex gap-4">
                    <div className="w-10 h-10 bg-blue-100 dark:bg-[#d4af37]/10 rounded-full flex items-center justify-center shrink-0">
                      <Info size={20} className="text-blue-500 dark:text-[#d4af37]" />
                    </div>
                    <div className="flex-1">
                      <h4 className="text-sm font-bold text-blue-900 dark:text-[#d4af37] mb-1">Información Importante</h4>
                      <p className="text-xs text-blue-700 dark:text-gray-400 leading-relaxed font-medium">
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-[#d4af37] hover:bg-[#b8962d] text-black font-bold rounded-2xl transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg shadow-[#d4af37]/20"
                  >
