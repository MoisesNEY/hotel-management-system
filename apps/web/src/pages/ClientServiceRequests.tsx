...
const ClientServiceRequests: React.FC = () => {
  ...
  return (
    <div className="text-white selection:bg-[#d4af37]/30">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">

        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
          <div>
            <h1 className="text-4xl font-light mb-2">Solicitar <span className="text-[#d4af37] font-semibold">Servicios</span></h1>
            <p className="text-gray-400">Personaliza tu estancia con nuestros servicios exclusivos de Room Service y Amenities.</p>
          </div>
          <button
        ...
        {bookings.length === 0 ? (
          <div className="bg-[#111111] border border-white/5 rounded-3xl p-12 text-center mb-12">
            <AlertCircle className="mx-auto text-yellow-500 mb-4" size={48} />
            <h3 className="text-2xl font-light mb-2">Acceso Restringido</h3>
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full bg-[#111111] border border-white/10 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-[#d4af37]/50 transition-all"
                  value={searchTerm}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredServices.map(service => (
                  <div key={service.id} className="bg-[#111111] border border-white/5 rounded-3xl p-6 hover:border-[#d4af37]/30 transition-all group">
                    <div className="flex items-start justify-between mb-4">
                      <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-[#d4af37] group-hover:scale-110 transition-transform">
                        {service.imageUrl ? <img src={service.imageUrl} className="w-full h-full object-cover rounded-2xl" /> : <Coffee size={24} />}
                      </div>
                      <div className="text-right">
                        <span className="text-xs text-gray-500 uppercase tracking-widest">Precio</span>
                        <p className="text-[#d4af37] font-bold">${service.cost}</p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-white/5 hover:bg-[#d4af37] hover:text-black transition-all rounded-xl font-semibold flex items-center justify-center gap-2"
                    >
                  ...
                  {myRequests.length === 0 ? (
                    ...
                    <div key={req.id} className="p-4 bg-white/5 rounded-2xl border border-white/5">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${req.status === 'COMPLETED' ? 'bg-green-500/20 text-green-400' :
                      ...
                      <p className="text-xs text-gray-500 line-clamp-2 mb-2">{req.details}</p>
                      <span className="text-[10px] text-gray-600 uppercase tracking-tighter">{new Date(req.requestDate).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {showModal && selectedService && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="bg-[#111111] border border-white/10 w-full max-w-xl rounded-3xl overflow-hidden relative z-10 animate-in zoom-in-95 duration-200">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <h3 className="text-2xl font-light">Confirmar <span className="text-[#d4af37] font-semibold">Solicitud</span></h3>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                <div className="bg-white/5 p-4 rounded-2xl flex items-center gap-4 border border-white/5">
                  ...
                  <div>
                    <p className="text-xs text-gray-500 font-medium tracking-widest uppercase">Servicio seleccionado</p>
                    <p className="text-lg font-semibold">{selectedService.name} - <span className="text-[#d4af37]">${selectedService.cost}</span></p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <select
                        className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-4 appearance-none focus:outline-none focus:border-[#d4af37]/50"
                        value={selectedBookingId}
                        onChange={(e) => setSelectedBookingId(e.target.value)}
                        required
                      >
                        {bookings.map(b => (
                          <option key={b.id} value={b.id} className="bg-[#111111]">Reserva #{b.code} (Hab {b.items[0]?.assignedRoomNumber || 'N/A'})</option>
                        ))}
                      </select>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-4 min-h-[120px] focus:outline-none focus:border-[#d4af37]/50"
                ...
                <div className="bg-[#d4af37]/5 border border-[#d4af37]/10 p-4 rounded-2xl flex gap-3">
