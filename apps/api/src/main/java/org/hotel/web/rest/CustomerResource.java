...
/**
 * REST controller for managing {@link org.hotel.domain.Customer}.
 */
@RestController
...
public class CustomerResource {
    ...
    private static final String ENTITY_NAME = "hotelBackendCustomer";
    ...
    public ResponseEntity<List<CustomerDTO>> getAllCustomers(
        ...
        if (licenseId != null && !licenseId.isEmpty()) {
            Optional<CustomerDTO> match = customerService.findOneByLicenseId(licenseId);
            if (match.isPresent()) {
                page = new PageImpl<>(List.of(match.get()), pageable, 1);
            } else {
